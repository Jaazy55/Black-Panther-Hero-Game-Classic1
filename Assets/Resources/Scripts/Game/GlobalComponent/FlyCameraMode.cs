...
namespace Game.GlobalComponent
{
	...
	public class FlyCameraMode : CameraMode
	{
		public override Game.Character.Modes.Type Type
		{
			get
			{
				return Game.Character.Modes.Type.Fly;
			}
		}
		...
		public override void Init()
		{
			...
			this.config = base.GetComponent<FlyConfig>();
			this.config.Awake();
		...
		private void UpdateDir()
		{
			...
			{
				float x = Quaternion.LookRotation(this.velocityFilter.GetValue()).eulerAngles.x;
				this.rotY = -x * 0.0174532924f;
			...
			float float2 = this.config.GetFloat("SpeedRollback");
			this.rollbackDistance = @float * Mathf.Clamp01(this.velocityFilter.GetValue().magnitude * float2);
		...
		private void UpdateAutoReset()
		{
			if (this.config.GetBool("AutoReset"))
			{
				...
				if (this.resetTimeout <= 0f)
				{
					...
					float num2 = -Quaternion.LookRotation(this.velocityFilter.GetValue()).eulerAngles.x;
					num2 *= 0.0174532924f;
					Vector3 toDirection;
					Game.Character.Utils.Math.ToCartesian(num, num2, out toDirection);
					Vector3 vector;
		...
		public override void GameUpdate()
		{
			...
			Vector3 a = this.targetPos - this.lastTargetPos;
