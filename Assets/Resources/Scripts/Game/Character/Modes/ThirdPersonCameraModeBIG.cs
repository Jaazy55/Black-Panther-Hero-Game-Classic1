...
{
	[RequireComponent(typeof(ThirdPersonConfigBIG))]
	public class ThirdPersonCameraModeBIG : CameraMode
	{
		public override Type Type
		{
			get
			{
				return Type.ThirdPersonBig;
			}
		}
		...
		public override void Init()
		{
			...
			this.config = base.GetComponent<ThirdPersonConfigBIG>();
		...
		private void UpdateAutoReset(ref Vector3 dir)
		{
			if (this.AutoReset)
			{
				...
				if (this.resetTimeout < 0f && this.hasRotated)
				{
					...
					Quaternion rotation = Quaternion.Slerp(Quaternion.identity, b, Time.deltaTime * this.AutoResetSpeed * 10f);
					dir = rotation * dir;
				}
			}
		}
		...
		public void UpdateAutoAim()
		{
			...
			{
				this.UnityCamera.transform.forward = Vector3.Lerp(this.UnityCamera.transform.forward, autoAimTarget.position + targetLocalOffset - this.UnityCamera.transform.position, this.AutoAimSensitivity * Time.deltaTime);
				this.currentAutoAimDuration += Time.deltaTime;
